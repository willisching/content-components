	...
	middlewares: [
		function mockContentServiceApis(ctx, next) {
			if (ctx.url.startsWith('/contentservice/api/97dc5895-cd3b-465f-8d5b-3c8d7e1bb6a7/search/')) {
				...
				ctx.status = 200;
			} else if (ctx.url.startsWith('/contentservice/api/97dc5895-cd3b-465f-8d5b-3c8d7e1bb6a7/content')) {
				ctx.body = {
				...
				ctx.status = 200;
			} else if (ctx.url.startsWith('/contentservice/api/97dc5895-cd3b-465f-8d5b-3c8d7e1bb6a7/topic')) {
				ctx.body = {};
				ctx.status = 200;
			}
