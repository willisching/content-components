...
class Producer extends RtlMixin(InternalLocalizeMixin(LitElement)) {
	...
	static get styles() {
		return [bodyStandardStyles, labelStyles, selectStyles, css`
			.d2l-producer-no-editor-container {
				align-items: center;
				display: flex;
			...
			.d2l-producer-processing-message {
				text-align: center;
			}


			.d2l-producer-processing-failed-message {
				color: var(--d2l-color-feedback-error);
				text-align: center;
			}

			.d2l-producer-top-bar-controls {
				align-items: center;
				display: flex;
			...
			.d2l-producer-saved-unsaved-indicator-container {
				align-items: center;
				display: flex;
				flex-direction: row;
				margin-right: 15px;
			...
			.d2l-producer-controls-optimize-for-streaming-button {
				margin-right: auto;
			}

			.d2l-producer-saved-unsaved-indicator-icon {
				margin-left: 5px;
			}

			.d2l-producer-controls-revision-dropdown {
				margin-right: 10px;
			}

			.d2l-producer-controls-publish-button {
				margin-left: 10px;
			}

			.d2l-producer-controls-publish-button .d2l-producer-controls-finishing {
				display: flex;
				align-items: center;
			}

			.d2l-producer-controls-publish-button d2l-loading-spinner {
				margin-right: 5px;
			}

			.d2l-producer-controls-save-button .d2l-producer-controls-saving {
				display: flex;
				align-items: center;
			}

			.d2l-producer-controls-save-button d2l-loading-spinner {
				margin-right: 5px;
			}
		`];
	}
	...
	async connectedCallback() {
		...
		// we only need resizing for case when producer is opened in the dialog
	...
	render() {
		...
		return html`
			<div class="d2l-producer">
				...
				></d2l-producer-editor>
				<d2l-dialog-confirm
					class="d2l-producer-dialog-confirm-copy-from-revision"
				...
				<d2l-dialog-confirm
					class="d2l-producer-dialog-confirm-change-language"
				...
				<d2l-dialog-confirm
					class="d2l-producer-dialog-confirm-finish"
	...
	get _editor() {
		return this.shadowRoot && this.shadowRoot.querySelector('d2l-producer-editor');
	}
	...
	_handleFinish() {
		this.shadowRoot.querySelector('.d2l-producer-dialog-confirm-finish').open();
	}
	...
	_handleSelectedLanguageChanged(e) {
		...
		if (this._captionsChanged) {
			this.shadowRoot.querySelector('.d2l-producer-dialog-confirm-change-language').open();
		} else {
	...
	async _handleSelectedRevisionChanged(e) {
		...
		if (this._unsavedChanges) {
			this.shadowRoot.querySelector('.d2l-producer-dialog-confirm-copy-from-revision').open();
		} else {
	...
	_renderLoadingIndicator() {
		return html`
			<div class="d2l-producer-no-editor-container">
	...
	_renderProcessingFailedMessage() {
		return html`
			<div class="d2l-producer-no-editor-container">
				...
				></d2l-icon>
				<p class="d2l-body-standard d2l-producer-processing-failed-message">${this.localize('revisionProcessingFailed')}</p>
			</div>
		`;
	}
	...
	_renderProcessingMessage() {
		return html`
			<div class="d2l-producer-no-editor-container">
				...
				<p class="d2l-body-standard d2l-producer-processing-message">${this.localize('revisionProcessing')}</p>
			</div>
		`;
	}
	...
	_renderSavedUnsavedIndicator() {
		...
		return html`
		  <div class="d2l-producer-saved-unsaved-indicator-container">
			<p class="d2l-label-text" style="color: ${feedbackColor};">
				${this.localize(langterm)}
			</p>
			<d2l-icon class="d2l-producer-saved-unsaved-indicator-icon" icon="${icon}" style="color: ${feedbackColor};"></d2l-icon>
		  </div>`;
	}
	...
	_renderTopBar() {
		...
		return html`
			<div class="d2l-producer-top-bar-controls">
${ showOptimizeForStreamingButton ?
		html`
		<d2l-button
			class="d2l-producer-controls-optimize-for-streaming-button"
		...
		${this.localize('optimizeForStreaming')}
				...
				<d2l-dropdown-button-subtle
					class="d2l-producer-controls-revision-dropdown"
				...
				<d2l-button
					class="d2l-producer-controls-save-button"
					@click="${this._handleSave}"
					?disabled="${this._saveIsDisabled}"
					text="${this.localize('saveDraft')}"
				>
					<div class="d2l-producer-controls-saving" style="${!this._saving ? 'display: none' : ''}">
				...
				<d2l-button
					...
					class="d2l-producer-controls-publish-button"
					primary
				><div class="d2l-producer-controls-finishing" style="${!this._finishing ? 'display: none' : ''}">
						<d2l-loading-spinner size="20"></d2l-loading-spinner>
...
customElements.define('d2l-producer', Producer);
