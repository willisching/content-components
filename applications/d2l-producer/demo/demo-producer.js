...
class DemoVideoProducer extends LitElement {
	...
	static get styles() {
		return [labelStyles, radioStyles, css`
			...
			.demo-producer-loading-container {
				display: flex;
				flex-direction: row;
			...
			.demo-producer-content-type-selection {
			...
			.demo-producer-content-type-selection label {
				margin-right: 10px;
			}

			.demo-producer-controls {
				align-items: center;
				display: flex;
			...
			.demo-producer-saved-unsaved-indicator-container {
				align-items: center;
				display: flex;
				flex-direction: row;
				margin-right: 15px;
			...
			.demo-producer-saved-unsaved-indicator-icon {
				margin-left: 5px;
			}

			.demo-producer-controls-publish-button {
				margin-left: 10px;
			}

			.demo-producer-controls-publish-button .demo-producer-controls-publishing {
				display: flex;
				align-items: center;
			}

			.demo-producer-controls-publish-button demo-loading-spinner {
				margin-right: 5px;
			}
		`];
	}
	...
	render() {
		if (this._loading) {
			return html`<div class="demo-producer-loading-container"><d2l-loading-spinner size=150></d2l-loading-spinner></div>`;
		}
		return html`
			<div class="demo-producer">
				<p class="d2l-label-text">Content Type</p>
				<div class="demo-producer-content-type-selection">
				...
				<div class="demo-producer-controls">
					${this._renderSavedUnsavedIndicator()}
					<d2l-button
						class="demo-producer-controls-save-button"
					...
					<d2l-button
						...
						class="demo-producer-controls-publish-button"
						primary
					><div class="demo-producer-controls-publishing" style="${!this._finishing ? 'display: none' : ''}">
	...
	async fetchAndSetData() {
		...
		setTimeout(() => {
			this.metadata = {
				...
				cuts: [{ in: 10, out: 60 }, { in: 200, out: 320 }, { in: 515 }],
			};
	...
	_renderSavedUnsavedIndicator() {
		...
		return html`
		  <div class="demo-producer-saved-unsaved-indicator-container">
			<p class="d2l-label-text" style="color: ${feedbackColor};">
				${label}
			</p>
			<d2l-icon class="demo-producer-saved-unsaved-indicator-icon" icon="${icon}" style="color: ${feedbackColor};"></d2l-icon>
		  </div>`;
	}
}

customElements.define('d2l-producer-demo', DemoVideoProducer);
