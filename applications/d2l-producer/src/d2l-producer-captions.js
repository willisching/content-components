...
import './d2l-producer-auto-generate-captions-dialog';
...
class CaptionsCueListItem extends InternalLocalizeMixin(LitElement) {
	...
	static get styles() {
		return [ bodyStandardStyles, inputStyles, labelStyles, css`
			.d2l-producer-captions-cues-list-item {
			...
			.d2l-producer-captions-cues-list-item-active {
				background-color: var(--d2l-color-gypsum);
			...
			.d2l-producer-captions-cue-main-controls {
				align-items: center;
				display: flex;
				flex-direction: row;
			...
			.d2l-producer-captions-cue-text-input {
				margin-bottom: 10px;
				margin-top: 12px;
				width: 100%;
			...
			.d2l-producer-captions-cue-main-controls-buttons {
				align-items: center;
				display: flex;
				flex-direction: column;
			}

			.d2l-producer-captions-cue-expanded-controls {
				align-items: flex-end;
				display: flex;
				flex-direction: row;
			}

			.d2l-producer-captions-cue-start-end-grid {
				align-items: center;
				display: grid;
				justify-content: space-between;
				padding-left: 2px;
			...
			.d2l-producer-captions-cue-start-end-grid > p {
				margin-top: 0px;
				margin-bottom: 0px;
			}
			...
			.d2l-producer-captions-cue-timestamp-label {
				grid-column: 1;
			}

			.d2l-producer-captions-cue-timestamp-input {
				grid-column: 2;
				margin-left: auto;
				margin-right: auto;
				width: 90%;
			...
			.d2l-producer-captions-sync-cue-button {
				grid-column: 3;
			}

			.d2l-producer-captions-error-icon {
				grid-column: 2;
			}
			` ];
	}
	...
	render() {
		return html`
		<div
			class="d2l-producer-captions-cues-list-item${this.active ? '-active' : ''}"
			>
	...
	_renderExpandedControls() {
		...
		return html`
			<div class="d2l-producer-captions-cue-expanded-controls">
				<div class="d2l-producer-captions-cue-start-end-grid">
					<p class="start-timestamp-row d2l-label-text d2l-producer-captions-cue-timestamp-label">${this.localize('captionsCueStartTimestamp')}</p>
					<p class="start-timestamp-row d2l-body-standard d2l-producer-captions-cue-timestamp-input">
						<d2l-input-text
					...
					<d2l-button-icon
						...
						class="start-timestamp-row d2l-producer-captions-sync-cue-button"
						@click="${this._handleSyncStartTimestampClicked}"
						text=${this.localize('syncStartTimeToTimeline')}
						icon="tier1:time"
					></d2l-button-icon>
					<p class="end-timestamp-row d2l-label-text d2l-producer-captions-cue-timestamp-label">${this.localize('captionsCueEndTimestamp')}</p>
					<p class="end-timestamp-row d2l-body-standard d2l-producer-captions-cue-timestamp-input">
						<d2l-input-text
					...
					<d2l-button-icon
						...
						class="end-timestamp-row d2l-producer-captions-sync-cue-button"
	...
	_renderMainControls() {
		return html`
			<div class="d2l-producer-captions-cue-main-controls">
				<textarea
					aria-label=${this.localize('captionsCueText')}
					class="d2l-input d2l-producer-captions-cue-text-input"
					@focus="${this._handleFocus}"
					@input="${this._handleTextInput}"
					maxlength="${constants.MAX_CAPTIONS_CUE_CHARACTERS}"
					rows="2"
				>${this.cue.text}</textarea>
				<div class="d2l-producer-captions-cue-main-controls-buttons">
					<d2l-button-icon
...
customElements.define('d2l-producer-captions-cues-list-item', CaptionsCueListItem);
...
class VideoProducerCaptions extends InternalLocalizeMixin(LitElement) {
	...
	static get styles() {
		return [ labelStyles, css`
			...
			.d2l-producer-captions-editor-top-buttons {
			...
			.d2l-producer-no-captions-message {
				margin-left: 20px;
				margin-right: 20px;
				margin-top: 65%;
				text-align: center;
			...
			.d2l-producer-captions-cues-list {
				display: flex;
				flex-direction: column;
			...
			.d2l-producer-preparing-captions-overlay {
				align-items: center;
			...
			.d2l-producer-captions-bottom-bar {
				background-color: white;
	...
	render() {
		return html`
			<div class="d2l-producer-captions">
				${this._renderContent()}
				<d2l-alert-toast
					id="d2l-producer-captions-alert-toast"
				></d2l-alert-toast>
			</div>
		`;
	}
	...
	_onUploadClicked() {
		this.shadowRoot.querySelector('#d2l-producer-captions-file-uploader').click();
	}

	_openAlertToast({type, text}) {
		const alertToast = this.shadowRoot.querySelector('#d2l-producer-captions-alert-toast');
	...
	_renderBottomBar() {
		return html`
			<div class="d2l-producer-captions-bottom-bar">
				<d2l-button-icon
					class="d2l-producer-captions-add-cue-button"
				...
				></d2l-input-text>
				<d2l-button-icon
					class="d2l-producer-captions-clear-all-button"
		...
		if (this.loading) {
			...
			return html`
				...
				<input
					...
					id="d2l-producer-captions-file-uploader"
					style="display: none;"
					type="file"
				></input>
				<d2l-producer-auto-generate-captions-dialog
	...
	_renderCuesList() {
		...
		html`<div class="d2l-producer-captions-cues-list">
	...
	_renderEmptyCaptionsMessage() {
		return html`
			<p class="d2l-body-standard d2l-producer-no-captions-message">${this.localize('noCaptions', { language: this.selectedLanguage.name })}</p>
		`;
	}
	...
	_renderPreparingCaptionsOverlay() {
		return html`
			<div class="d2l-producer-preparing-captions-overlay">
	...
	_renderTopButtons() {
		return html`
			<div class="d2l-producer-captions-editor-top-buttons">
				<d2l-button-subtle
	...
	async _scrollToActiveCue() {
		...
		let captionsCuesList = this.shadowRoot.querySelector('.d2l-producer-captions-cues-list');
		if (!(captionsCuesList?.childNodes?.length >= this._numberOfVisibleCuesInList)) {
			const waitUntilListIsReady = resolve => {
				captionsCuesList = this.shadowRoot.querySelector('.d2l-producer-captions-cues-list');
				if (captionsCuesList?.childNodes?.length >= this._numberOfVisibleCuesInList) {
					resolve();
				} else {
...
customElements.define('d2l-producer-captions', VideoProducerCaptions);
