...
class VideoProducerChapters extends InternalLocalizeMixin(LitElement) {
	...
	static get styles() {
		return [ bodySmallStyles, bodyStandardStyles, heading3Styles, css`
			...
			.d2l-producer-chapters-heading {
				margin: 0;
				padding: 10px 10px 0;
			}

			.d2l-producer-chapters-heading .d2l-body-small.hidden {
				visibility: hidden;
			}

			.d2l-producer-chapters-container {
				display: flex;
			...
			.d2l-producer-chapter {
				display: flex;
			}

			.d2l-producer-chapter-time {
			...
			.d2l-producer-chapter-time.active-chapter {
				color: var(--d2l-color-celestine);
			}

			.d2l-producer-chapter-set-chapter-time-button {
				margin-right: 7px;
			}

			.d2l-producer-new-chapter-footer {
				display: flex;
				padding: 4px;
				text-align: center;
			}

			.d2l-producer-new-chapter-button,
			.d2l-producer-new-chapter-title-input {
				padding: 3px;
			}
		`];
	}
	...
	render() {
		return html`
			<div class="d2l-producer-chapters">
				<h3 class="d2l-heading-3 d2l-producer-chapters-heading ${this._editingOverrides ? '' : 'hidden'}">
					<div class="d2l-body-small">
						${this.localize('editingOverrides', { language: this.selectedLanguage && this.selectedLanguage.name })}
					</div>
				</h3>
				<div class="d2l-producer-chapters-container">
					${this._renderChapters()}
				</div>
				<div class="d2l-producer-new-chapter-footer">
					<d2l-button-icon
						?disabled="${!this._newChapterTitle}"
						@click=${this._addNewChapter}
						class="d2l-producer-new-chapter-button"
					...
					<d2l-input-text
						...
						class="d2l-producer-new-chapter-title-input"
	...
	addNewChapter(chapterTime) {
		...
		this._newChapterTitle = '';
		this.shadowRoot.querySelector('.d2l-producer-new-chapter-title-input').value = '';
	}
	...
	setChapterToTime(chapterTime) {
		...
		if (indexOfUpdatedChapter > -1) {
			...
			const inputTextOfUpdatedChapter = [...this.shadowRoot
				.querySelectorAll('.d2l-producer-chapter d2l-input-text')
	...
	_renderChapters() {
		...
		return this._sortedChapters.map(({ time, title, originalIndex }) => {
			...
			return html`
				<div class="d2l-producer-chapter">
					<d2l-input-text
						...
						label-hidden
					></d2l-input-text>
					<p class="${this._activeChapterIndex === originalIndex ? 'active-chapter' : ''} d2l-producer-chapter-time">
						${this._getTime(time)}
					</p>
					<d2l-button-icon
						@click="${this._setChapterToCurrentTime(originalIndex)}"
						class="d2l-producer-chapter-set-chapter-time-button"
						icon="tier1:time"
...
customElements.define('d2l-producer-chapters', VideoProducerChapters);
