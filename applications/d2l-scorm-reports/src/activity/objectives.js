...
class Objectives extends LitElement {
	...
	constructor() {
		...
		this.tableSortInfo = [
			...
			{header: 'progress', sortBy: (entry) => entry.progress},
		];
	...
	async connectedCallback() {
		...
		const tableInfo = await this.client.reports.getObjectives({contextId: this.contextId, topicId: this.topicId, userId: this.userId});
	...
	async getCsv() {
		const response = await this.client.reports.getObjectives({contextId: this.contextId, topicId: this.topicId, userId: this.userId, csv: true});
		return await response.text();
	}
}

customElements.define('d2l-activity-objectives', Objectives);
