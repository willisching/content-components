...
class RecycleBinItem extends DependencyRequester(navigationMixin(InternalLocalizeMixin(LitElement))) {
	...
	static get styles() {
		return [bodyCompactStyles, bodySmallStyles, css`
			...
			.d2l-content-library-deleted-item-illustration {
				align-items: center;
				display: flex;
				height: 64px;
			...
			.d2l-content-library-deleted-poster-image {
				height: 100%;
				object-fit: cover;
				width: 100%;
			}
	...
	destroyHandler() {
		return async() => {
			await this.shadowRoot.querySelector('.d2l-content-library-confirm-destroy').open();
		};
	}
